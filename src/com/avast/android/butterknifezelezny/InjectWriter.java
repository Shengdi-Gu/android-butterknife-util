...
public class InjectWriter extends WriteCommandAction.Simple {
    ...
    protected boolean mRToR2;

    public InjectWriter(PsiFile file, PsiClass clazz, String command, ArrayList<Element> elements, String layoutFileName, String fieldNamePrefix, boolean createHolder, boolean splitOnclickMethods, boolean needInit, boolean rToR2) {
    ...
    private void generateSingleClickMethod() {
        ...
        for (Element element : mElements) {
            if (element.isClick) {
                method.append(mRToR2 ? element.getFullID().replace("R.", "R2.") : element.getFullID()).append(")");
            }
        }
    ...
    private void generateMultipleClickMethods() {
        for (Element element : mElements) {
            if (element.isClick) {
                StringBuilder method = new StringBuilder();
                method.append("@butterknife.OnClick(").append(mRToR2 ? element.getFullID().replace("R.", "R2.") : element.getFullID()).append(")");
                method.append("public void on").append(Utils.capitalize(element.fieldName)).append("Clicked() {}");
                mClass.add(mFactory.createMethodFromText(method.toString(), mClass));
            }
        }
    }
    ...
    private void generateSingleClickMethodForSeveralIds() {
        ...
        if (mRToR2) {
            method.append("int i = view.getId();\n");
            currentCount = 0;
            for (Element element : mElements) {
                if (element.isClick) {
                    currentCount++;
                    if (currentCount == 1) {
                        method.append("if (i == ").append(element.getFullID()).append(") {}");
                    } else {
                        method.append("else if (i == ").append(element.getFullID()).append(") {}");
                    }
                }
            }
        } else {
...
//        mClass.addBefore(mFactory.createCommentFromText(comment.toString(), mClass), mClass.findInnerClassByName(Utils.getViewHolderClassName(), true));
    ...
    protected void generateFields(@NotNull IButterKnife butterKnife) {
        ...
        for (Element element : mElements) {
            ...
            injection.append(butterKnife.getFieldAnnotationCanonicalName());
            injection.append('(');
            injection.append(mRToR2 ? element.getFullID().replace("R.", "R2.") : element.getFullID());
            injection.append(") ");
            if (element.nameFull != null && element.nameFull.length() > 0) { // custom package+class
                injection.append(element.nameFull);
            } else if (Definitions.paths.containsKey(element.name)) { // listed class
    ...
    protected void generateInjects(@NotNull IButterKnife butterKnife) {
        ...
        //添加对androidx的Fragment支持
        PsiClass androidxFragment = JavaPsiFacade.getInstance(mProject).findClass(
        ...
        if (activityClass != null && mClass.isInheritor(activityClass, true)) {
            ...
            // Check for Fragment class
                ...
                || (androidxFragment != null && mClass.isInheritor(androidxFragment, true))) {
